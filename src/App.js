...
function App() {
  const [modelConfigs, setModelConfigs] = useState([
    {
      ...
      prompts: [
        { id: Date.now(), systemPrompt: "", userPrompt: "", output: "" },
      ],
    },
  ]);
  ...
  const savePrompt = () => {
    if (promptNameToSave) {
      ...
      const newSavedPrompts = [
        ...savedPrompts,
        { name: promptNameToSave, content: promptContent },
      ];
  ...
  const saveModel = () => {
    if (modelNameToSave && modelToSave) {
      ...
      const newSavedModels = [
        ...savedModels,
        { name: modelNameToSave, config: modelToSaveWithoutId },
      ];
  ...
  const saveVariables = () => {
    if (variablesNameToSave) {
      const newSavedVariables = [
        ...savedVariables,
        { name: variablesNameToSave, variables: variables },
      ];
  ...
  const runPrompt = async (modelId, promptId) => {
    const model = modelConfigs.find((m) => m.id === modelId);
    const prompt = model.prompts.find((p) => p.id === promptId);
    let processedSystemPrompt = useGlobalPrompt
      ? globalSystemPrompt
      : prompt.systemPrompt;
    let processedUserPrompt = useGlobalPrompt
      ? globalUserPrompt
      : prompt.userPrompt;
    variables.forEach((v) => {
  ...
  return (
    <Box sx={{ height: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <PanelGroup direction="horizontal" style={{ flexGrow: 1 }}>
            ...
            <List>
              {savedPrompts.map((savedPrompt, index) => (
                <ListItem key={index} disablePadding sx={{ mb: 1 }}>
                  ...
                  <IconButton onClick={() => removeSavedPrompt(index)}>
                    <DeleteIcon />
                  </IconButton>
                </ListItem>
              ))}
        ...
        <PanelResizeHandle
          style={{ width: "8px", background: "#f0f0f0", cursor: "col-resize" }}
        />
        <Panel>
          <Box sx={{ height: "100%", overflowY: "auto", p: 2 }}>
            <Paper sx={{ p: 2, mb: 2 }}>
              ...
              >
                ...
                <Button
                  variant="outlined"
                  onClick={() => openSavePromptDialogHandler(null, null)}
                  disabled={!useGlobalPrompt}
                >
                  Save Global Prompt
                </Button>
                    ...
                    <FormControl fullWidth sx={{ mb: 2 }}>
                      ...
                      >
                        ...
                        {savedModels.map((savedModel, index) => (
                          <MenuItem key={index} value={savedModel.name}>
                            {savedModel.name}
                          </MenuItem>
                        ))}
                      </Select>
                        ...
                        {config.prompts.map((prompt) => (
                          <Box key={prompt.id} sx={{ mb: 2 }}>
                            ...
                            <Button
                              variant="outlined"
                              onClick={() =>
                                handleDeletePrompt(config.id, prompt.id)
                              }
          ...
          <Box sx={{ height: "100%", overflowY: "auto", p: 2 }}>
            <Paper sx={{ p: 2, mb: 2 }}>
              ...
              >
                ...
                <Button
                  variant="outlined"
                  onClick={openSaveVariablesDialogHandler}
                  startIcon={<SaveIcon />}
                >
                  Save Variables
                </Button>
              ...
              <List>
                {savedVariables.map((savedVarSet, index) => (
                  <ListItem key={index} disablePadding sx={{ mb: 1 }}>
                    <Button
                      variant="outlined"
                      onClick={() => loadVariables(savedVarSet)}
                      sx={{ mr: 1, flexGrow: 1 }}
                    >
                      {savedVarSet.name}
                    </Button>
                    <IconButton onClick={() => removeSavedVariables(index)}>
                      <DeleteIcon />
                    </IconButton>
                  </ListItem>
                ))}
      ...
      <Dialog
        ...
        onClose={() => setOpenSaveModelDialog(false)}
      >
      ...
      <Dialog
        open={openSaveVariablesDialog}
        onClose={() => setOpenSaveVariablesDialog(false)}
      >
        ...
        <DialogActions>
          <Button onClick={() => setOpenSaveVariablesDialog(false)}>
            Cancel
          </Button>
